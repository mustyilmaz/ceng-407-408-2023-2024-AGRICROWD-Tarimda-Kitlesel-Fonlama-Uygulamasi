...
function AdminHome() {
  ...
  // Function to handle withdrawal of commission funds
  const withdrawCommissionFunds = async () => {
    try {
      // Create a provider and signer for ethers.js
      const provider = new ethers.providers.Web3Provider(window.ethereum);
      const signer = provider.getSigner();
      ...
      // Withdraw commission funds from the contract
      await contract.withdrawCommissionFunds();

      // Fetch the updated total commission amount from the contract
      const totalCommissionFromContract =
        await contract.getTotalCommissionAmount();
      ...
      // Update the total commission amount in state
      setTotalCommission(totalCommissionInEther);
    } catch (error) {
      console.error("Error withdrawing commission funds:", error);
    }
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <div>
        ...
        <h2 className={styles.withdrawButton}>
